...
int adventurerEffect(struct gameState *state, int handPos) { 
  ...
  while(drawntreasure < 2) { 
    ...
    //if (state->deckCount[currentPlayer] > 1) { //mutant to achieve 100% coverage
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) { 
  ...
  switch(card) { 
    case adventurer:
      return adventurerEffect(state, handPos);

    case council_room:
      return councilRoomEffect(state, handPos);

    case feast:
      return feastEffect(choice1, state, handPos);

    case gardens:
      return -1;
			
    case mine:
      return mineEffect(choice1, choice2, state, handPos);
